...
for ($i = 1; $i -le 4; $i++) {
    ...
    Start-Process powershell -ArgumentList "-NoExit", "-Command", "Write-Host 'Starting Member $i...' -ForegroundColor Yellow; mvn exec:java '-Dexec.mainClass=com.example.Member' '-Dexec.args=member$i'"
...
Start-Process powershell -ArgumentList "-NoExit", "-Command", "Write-Host 'Starting ClientLibrary REST API on port $HttpPort...' -ForegroundColor Yellow; mvn exec:java '-Dexec.mainClass=com.example.ClientLibrary' '-Dexec.args=$HttpPort'"
